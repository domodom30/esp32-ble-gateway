...
private:
  ...
  static void sendServices(const uint8_t client, BLEPeripheralID id, const std::vector<NimBLERemoteService *> *services);
  static void sendCharacteristics(const uint8_t client, BLEPeripheralID id, std::string service, const std::vector<NimBLERemoteCharacteristic *> *characteristics);
